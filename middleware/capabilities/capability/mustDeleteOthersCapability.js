...
var mustDeleteOthersCapability = async (req, res, next) => {
  try {
    const theCheck = await ACL.isUserCan(req.user, ['deleteOthersCapability'])
    if (theCheck) {
      next()
    } else {
...
module.exports = { mustDeleteOthersCapability }
