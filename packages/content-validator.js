...
var ConditionalLogicValidator = require('./conditional-logic-validator')
...
class ContentValidator {
  ...
  static getSchemaVisibleFields (schemaFields, values, customFields) {
    ...
    const visibleContentFields = ConditionalLogicValidator.getVisibleFields(
      schemaFields,
      values
    )
    ...
    visibleContentFields.forEach((field, index) => {
      if (field.type === 'custom') {
        const customFieldData = customFields.find(e => e.name === field.name)
        ...
        if (field.repeatable) {
          ...
          for (const value of customFieldValues) {
            const customFieldVisibleFields = ConditionalLogicValidator.getVisibleFields(
  ...
  static validateFields (fields, values, customFields = null) {
    try {
      const getSchemaVisibleFields = this.getSchemaVisibleFields(
        fields,
        values,
        customFields
      )
      ...
      const schemaShape = this.getSchemaShape(getSchemaVisibleFields)

      const validationSchema = Joi.object(schemaShape)
